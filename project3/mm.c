...
int mm_init(void)
{
    ...
    PUT(heap_listp + (2*WSIZE), heap_listp+(WSIZE*3)); /* prev */
    PUT(heap_listp + (3*WSIZE), heap_listp+(WSIZE*2)); /* next */
...
static void *coalesce(void *bp)
{
   ...
   if (prev_alloc && next_alloc) {            /* Case 1 */
       ...
       deleteBlockFromList(NEXT_BLKP(bp));
       ...
       deleteBlockFromList(PREV_BLKP(bp));
   ...
   else {                                     /* Case 4 : next, prev are free */
       deleteBlockFromList(NEXT_BLKP(bp));
       deleteBlockFromList(PREV_BLKP(bp));
       size += GET_SIZE(HDRP(PREV_BLKP(bp))) +
...
/* $end mmfirstfit-proto */
{
    ...
    for (bp = NEXT_FREE(free_listp); GET_ALLOC(HDRP(bp)) != 1 ; bp = NEXT_FREE(bp)) {
...
/* $end mmplace-proto */
{
    ...
    deleteBlockFromList(bp); // delete from list
